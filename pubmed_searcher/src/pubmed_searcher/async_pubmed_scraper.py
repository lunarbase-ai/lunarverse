...
"""
Based on Ilia Zenkov's work from 9/26/2020
...
def get_num_pages(keyword: str, pages: int, pubmed_url: str):
    ...
    headers = make_header()
    search_url = f'{pubmed_url}+({keyword}[Title%2FAbstract])+OR+({keyword}[MeSH+Terms])'
...
async def build_article_urls(keywords: str, pages: int, start: int, stop: int, pubmed_url: str):
    ...
    :param keywords: string: search word used to search for results
    :return: None
    """
    tasks = []
    for keyword in keywords:
        num_pages = get_num_pages(keyword, pages, pubmed_url)
        for page in range(1,num_pages+1):
            task = asyncio.create_task(get_pmids(page, keyword, pubmed_url))
            tasks.append(task)
...
def main(pages: int, keywords: str, start: int = 2019, stop: int = 2023) -> pd.DataFrame:
    ...
    pubmed_url = f'https://pubmed.ncbi.nlm.nih.gov/?term={start}%3A{stop}%5Bdp%5D'
    ...
    search_keywords = [kw.strip() for kw in keywords]
    print(f'\nFinding PubMed article URLs for {len(keywords)} keywords\n')
    ...
    loop.run_until_complete(build_article_urls(search_keywords, pages, start, stop, pubmed_url))
    print(f'Scraping initiated for {len(urls)} article URLs found from {start} to {stop}\n')
    ...
    cols = ['title', 'abstract', 'affiliations', 'authors', 'journal', 'date', 'keywords', 'url']
    articles_df = pd.DataFrame(articles_data, columns=cols)
    # print('Preview of scraped article data:\n')
    # print(articles_df.head(5))
    print(f'It took {time.time() - time_start} seconds to find {len(urls)} articles; {len(scraped_urls)} unique articles were retrieved')
